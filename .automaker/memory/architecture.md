...
- **Context:** Watchdog should track anomalous _drops_ in feature count, not fluctuations from normal development (features added, others completed/archived)
...
#### [Pattern] Maintenance task integration pattern: registered task runs integrity check on _all_ active projects every 5 minutes, emits events for UI awareness (2026-02-10)
...
#### [Gotcha] ESM modules require import.meta.url with fileURLToPath() for directory resolution - no \_\_dirname available (2026-02-13)

- **Situation:** Initial approach assumed \_\_dirname would work in Node.js ESM, but CommonJS globals don't exist in ESM context
- **Root cause:** ESM spec doesn't provide **dirname/**filename. Must use import.meta.url (contains file:// URL) and convert to filesystem path with fileURLToPath() from 'url' module
...
#### [Gotcha] ES modules require explicit \_\_dirname polyfill using fileURLToPath(import.meta.url) + dirname(). This cannot be assumed to exist like in CommonJS. (2026-02-13)

- **Situation:** Package uses ES modules (type: module in package.json). Template path resolution failed because \_\_dirname was undefined at runtime.
- **Root cause:** Node.js ES modules don't provide \_\_dirname/filename globals. Must derive from import.meta.url which is only available in ES module scope, not CommonJS.
- **How to avoid:** Slightly more verbose imports (fileURLToPath, dirname) but guarantees correct path resolution in all ES module contexts. Alternative of assuming \_\_dirname exists creates hard-to-debug runtime failures.
...
### Crew member runs on 5-minute schedule (_/5 _ \* \* \*) for state drift detection, never escalates directly (2026-02-14)
...
### Configure Storybook story glob to scan monorepo paths (../src/\*_/_.stories.\*) instead of app-specific paths. This decouples story discovery from app location, allowing libs/ui stories to be found by name pattern alone. (2026-02-18)
...
- **Breaking if changed:** Removing the '../../../libs/ui/src/\*_/_.stories.@(js|jsx|mjs|ts|tsx)' entry from .storybook/main.ts stories array causes 25 stories to vanish from Storybook with no error message—only detection is 'fewer stories than expected'.
...
#### [Pattern] Retry counter nodes (increment\_\*\_retry) always loop back to phase start, creating cyclic patterns within linear graphs (2026-02-19)
...
#### [Pattern] Environment variables use consistent naming convention: UPPERCASE*WITH_UNDERSCORES for all vars, with prefixes for features (LANGFUSE*_, GITHUB\__, VITE*\*, AUTOMAKER*\*) to indicate subsystem ownership. Defaults are inline in CLAUDE.md (e.g., 'default: langfuse-sync') and .env.example is commented-out reference only. (2026-02-23)
...
- **Why this works:** Consistent naming enables shell scripts and config loaders to auto-discover vars by prefix (e.g., grep -E '^LANGFUSE\_' to find all Langfuse vars). Commented-out defaults in .env.example prevent accidental activation while documenting required vs optional. Inline defaults in CLAUDE.md serve as quick reference for developers.
...
#### [Pattern] Environment variable presence detection for feature selection (WIN*CSC_LINK vs AZURE_KEY_VAULT*\* vs unsigned builds) (2026-02-24)
...
#### [Gotcha] Automated issue filing systems (Linear integration) can mis-route features to wrong repositories when cross-product boundaries aren't strictly enforced (2026-02-25)

- **Situation:** Feature for MythxEngine (separate repository) was automatically created in Automaker project due to loose project mapping configuration
- **Root cause:** Linear integration relies on keywords/naming patterns to auto-create issues, but lacks awareness of which repository actually owns a product/codebase
- **How to avoid:** Automation saves time when configured correctly, but misconfiguration causes developers to investigate/start work on features belonging elsewhere; discovery cost is high

#### [Pattern] When feature context appears unfamiliar, verify code ownership through rapid pattern searching before committing implementation effort (2026-02-25)

- **Problem solved:** Searched for world-pack, worlds, and MythxEngine patterns across codebase - their complete absence immediately indicated feature misrouting vs infrastructure gaps
- **Why this works:** Prevents implementing features when required code structures don't exist in current repo; early detection saves hours vs discovering mid-implementation
- **Trade-offs:** 10-15 min upfront investigation is negligible cost vs wasted days on impossible feature; builds confidence in feature feasibility
...
#### [Pattern] Performance monitoring via `startTransaction()` on critical paths (feature execution in AutoModeService) with feature context automatically included (2026-02-25)

- **Problem solved:** Identifying slow or failing feature executions in production
- **Why this works:** Transactions group related operations and show performance bottlenecks; combined with feature context, you can see which features are problematic. Follows observability principle: measure the path users care about
- **Trade-offs:** Sampling rate must be tuned (high = costs, low = miss issues); adds small overhead to traced operations
