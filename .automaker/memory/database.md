...
- **Trade-offs:** Gained: 50-100x faster token calculations, no I/O. Lost: 5-15% accuracy vs real tokenizer (acceptable for budget buffer math).
...
- **Trade-offs:** Gained: Clean, synchronous API that matches SQLite semantics. Lost: Cannot parallelize I/O across network (but not applicable for local SQLite). Constraint: Must run in Node.js main thread or dedicated worker.
...
- **Trade-offs:** Gained: Better concurrency, predictable latency under load. Lost: Extra disk space for WAL files (negligible for conversation stores), requires cleanup on application shutdown.
...
- **Trade-offs:** Slightly more migration overhead vs. independence of schema and search index, easier to rebuild/fix FTS5 without touching core data
...
- **Trade-offs:** Redundant IF NOT EXISTS checks on every run vs. simplified deployment logic and no version tracking table. Performance negligible for schema operations.
...
- **Trade-offs:** Storage cost minimal, but timestamp queries don't need numeric conversion
...
- **Trade-offs:** Extra storage (~5% of content size) for better query performance and simpler exploration UX
...
- **Trade-offs:** Nullable foreign keys require careful NULL handling, but support broader use cases
...
- **Trade-offs:** Caller must handle OptimisticLockError and retry logic; version field grows unbounded over workflow lifetime
...
- **Trade-offs:** Version number grows unbounded; provides clear event ordering vs. version overflow risk (theoretical with reasonable workflow lifespans)
