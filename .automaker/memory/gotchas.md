...
#### [Gotcha] Cross-product architecture notes (memory files) contain stale file path references when code is removed or repositories are separated (2026-02-25)

- **Situation:** Memory files referenced examples/mythxengine/ directory containing MythxEngine code, but no examples/ directory exists - created false lead during investigation
- **Root cause:** Memory/knowledge base files are not automatically synchronized when directory structures change or when cross-product code references are removed
- **How to avoid:** Memory files accelerate investigation when current, but stale references cause time-wasting false leads; maintenance cost required

#### [Gotcha] Sentry v8 removed manual error handler middleware - automatic Express instrumentation handles error capture by default (2026-02-25)

- **Situation:** Implementation removed `Sentry.Handlers.errorHandler()` middleware from Express app
- **Root cause:** Sentry v8 switched to automatic instrumentation; manual middleware is not only unnecessary but keeping it alongside auto-instrumentation could cause double-capture and confusion
- **How to avoid:** Simpler setup (fewer middleware calls) but debugging error flow is less visible since instrumentation is automatic/hidden

#### [Gotcha] Sentry v8 `beforeSend` callback signature changed - different type signatures than v7, requires careful typing for privacy data modification (2026-02-25)

- **Situation:** Privacy implementation needed to modify event before transmission, but v8 changed how events are typed and processed
- **Root cause:** Sentry v8 refactored event types and processing pipeline; type assertions sometimes needed to work with evolving event data structure
- **How to avoid:** Slightly more complex type handling but ensures privacy rules actually work with v8's event structure
