...
#### [Pattern] Bounded DAG walk with BOTH tokenCap AND ttlMs constraints in expand tool (2026-03-16)
- **Problem solved:** Reconstructing original content from condensed/compacted context nodes with recursive parent traversal
- **Why this works:** Token explosion and time explosion are distinct failure modes. Token cap prevents context explosion. TTL prevents timeout hangs. Either alone insufficient based on earlier issues. Two independent failure modes require two independent guards.
- **Trade-offs:** More defensive but adds API complexity (two params); users must understand which constraint triggered early termination