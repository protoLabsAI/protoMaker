...
#### [Gotcha] Conditional test execution where test only runs if data exists causes tests to pass without testing. Template detail test only executes if templates are returned from list endpoint. (2026-02-22)
- **Situation:** The 'should get template details' test wraps its assertions in `if (listData.templates && listData.templates.length > 0)`, so it passes silently if no templates exist
- **Root cause:** Defensive programming to handle incomplete test environments, but creates false negatives where test passes without validating functionality
- **How to avoid:** More forgiving in various environments but masks failures; easier to run tests anywhere but harder to catch actual API problems

#### [Pattern] Using Playwright for lightweight API integration tests (smoke tests) that hit real endpoints with actual headers/auth, positioned as verification step between installation and next steps. (2026-02-22)
- **Problem solved:** Rather than pure unit tests or full E2E flows, smoke tests verify core functionality with minimal setup, documented in getting-started guide
- **Why this works:** Strikes balance between speed and confidence; real API testing catches integration issues unit tests miss, but faster than full E2E; documentable as customer-facing verification
- **Trade-offs:** Requires real API running (more dependencies) but catches real problems; slower than unit tests but faster than E2E; documents installation success to users

### Response bodies asserted with TypeScript `as` type annotations to document and enforce API contract shape, even though body content isn't fully validated. (2026-02-22)
- **Context:** Tests use patterns like `const data = (await response.json()) as { success: boolean; templates?: unknown[] }` to type-assert responses
- **Why:** Catches API contract changes at test time; documents expected response shape in test code; enables IDE type checking during test development
- **Rejected:** Could just check status codes and not validate body; could use runtime schema validation (zod/joi); could use any type
- **Trade-offs:** Tests become brittle to minor API changes but catch breaking changes early; adds documentation value; requires manual updates when API evolves
- **Breaking if changed:** Removing these assertions loses contract documentation and IDE type hints; tests no longer fail if API returns wrong shape

#### [Pattern] Establishing verification-first documentation workflow by adding test verification instructions to getting-started guide as explicit post-installation step before 'Next Steps'. (2026-02-22)
- **Problem solved:** Documentation structure adds 'Verifying Your Installation' section with code examples between installation and next steps in getting-started guide
- **Why this works:** Validates installation succeeded before user proceeds to more complex topics; creates explicit feedback loop; reduces support friction from incomplete installations
- **Trade-offs:** More steps for user but better confidence; establishes testing culture earlier; makes installation concrete/verifiable

### Fixed pre-existing build blocker (p-limit import) outside feature scope to satisfy verification gate requirements rather than requesting scope exception. (2026-02-22)
- **Context:** Primary feature (URL change) was simple and scoped, but unrelated package build failure prevented verification completion. Team faced choice: expand scope or request exception.
- **Why:** Verification gates are mandatory checkpoints. Expanding scope minimally (3 lines) to unblock gates was pragmatic; would have required external approval otherwise. Also prevents future builds from failing on this issue.
- **Rejected:** Request verification gate exception - would require additional approval and leave known blocker in codebase. Workaround-only approach - insufficient for proper verification.
- **Trade-offs:** Cleaner scope separation vs. guaranteed verification completion. Team chose verification completion, reducing risk of feature being blocked or reverted.
- **Breaking if changed:** Removing the p-limit fix re-introduces build failure. Any removal would require addressing the underlying ESM/CJS issue another way.

### Hardcoded test API key 'test-api-key-for-e2e-tests' instead of dynamic discovery or environment-only configuration (2026-02-22)
- **Context:** Authentication required for all /api/agents/* endpoints, tests need to work in multiple environments
- **Why:** Enables tests to have sensible defaults and run without additional setup, while maintaining deterministic behavior and test isolation from runtime API key generation
- **Rejected:** Could extract API key from server startup logs, config files, or require AUTOMAKER_API_KEY to always be set - but this couples test to server state or makes tests fail silently in new environments
- **Trade-offs:** Tests work out-of-box in controlled environments (CI, test runner) but fail gracefully in wrong environments (dev server with different key). Easy to debug vs portable.
- **Breaking if changed:** If this key is removed or validation logic changes, tests only work when API_KEY is properly configured externally. Removing the fallback exposes configuration assumptions.

#### [Pattern] Verify both HTTP status code AND response payload structure/format, not just status alone (2026-02-22)
- **Problem solved:** Tests need to validate entire API contract - a 404 with malformed JSON or a 200 with wrong fields both indicate failures
- **Why this works:** HTTP status only validates transport semantics, not data contract. A 404 with undefined error field fails the API contract even with correct status. Catches silent API evolution bugs.
- **Trade-offs:** More assertions and verbose test code, but catches real bugs. Tighter coupling to API schema.

### Separate verification tests in 'tests/verification/' directory instead of alongside E2E tests in main test suite (2026-02-22)
- **Context:** Smoke tests serve different purpose than E2E tests - quick health checks vs comprehensive flows
- **Why:** Enables different execution strategies: smoke tests can run first/fast/independently in CI, while full E2E tests run later. Clear separation of test tiers.
- **Rejected:** Could integrate into main E2E suite, but smoke tests would always run with full suite, slowing down quick feedback loops
- **Trade-offs:** Additional test directory structure to maintain, but faster subset execution. Enables staged testing (health check → full suite).
- **Breaking if changed:** If consolidated into main E2E, smoke tests slower to execute and CI feedback loop extends. Separate location enables independent execution.

#### [Gotcha] Test 2 uses conditional SKIP when templates unavailable instead of failing, masking setup issues (2026-02-22)
- **Situation:** If no templates exist at API, test silently passes instead of detecting missing precondition
- **Root cause:** Assumes templates might genuinely not exist in some scenarios, prioritizing robustness over strict verification. Prevents false failures.
- **How to avoid:** Flexible test won't break on empty state, but can hide missing setup. If templates are supposed to exist, missing them goes unnoticed.

#### [Pattern] Run all tests to completion before exiting with error code, rather than fast-fail on first error (2026-02-22)
- **Problem solved:** Need visibility into all test results in one run, especially for CI/CD reporting
- **Why this works:** Allows gathering full test results and pass/fail count before exit. CI/CD systems can see which tests passed/failed in single run. Better observability.
- **Trade-offs:** Slightly slower failure reporting but better test visibility and CI logging. More output to process.

#### [Gotcha] Verification gate dependencies: Feature verification surfaced pre-existing build blocker unrelated to feature scope (2026-02-23)
- **Situation:** Implementing trivial URL change required fixing p-limit import in unrelated secure-fs.ts to pass mandatory 'build must succeed' gate
- **Root cause:** Verification gates are chained - cannot verify feature without full package build succeeding. Pre-existing technical debt blocks feature completion if gate enforcement is strict.
- **How to avoid:** Stricter enforcement catches hidden issues but increases scope creep. Discovered pre-existing bug that would have remained unknown.

#### [Gotcha] Environmental constraint creates deliberate risk acceptance: Cannot run E2E tests when dev server already running on same port; guideline forbids managing dev server (2026-02-23)
- **Situation:** Playwright E2E test required to start test server on port 3008, but dev server already claimed port. CLAUDE.md forbids starting/stopping dev server.
- **Root cause:** Constraint prioritizes not disrupting developer environment over complete automated verification. Decision: Document risk explicitly rather than violate guideline by killing dev server.
- **How to avoid:** Accepts unverified E2E path but maintains guideline compliance. Relies on code inspection + build verification for trivial changes. Requires manual testing as mitigation.

#### [Pattern] E2E tests intentionally use separate server instances (reuseExistingServer: false) with known test API keys (test-api-key-for-e2e-tests) configured in playwright.config.ts, not from dev environment (2026-02-23)
- **Problem solved:** Need to verify API endpoints work correctly, but dev server uses different auth tokens and state. Tests must be reproducible and work in CI/CD where no dev server exists
- **Why this works:** Ensures test determinism and isolation - tests always use the same API key, same fresh server state, no cross-contamination from dev activity. Enables parallel test runs and CI/CD execution
- **Trade-offs:** Cannot run tests simultaneously with active dev server on same port (port 3008 collision), but gain complete test isolation and reproducibility

#### [Gotcha] E2E tests cannot execute when dev server is already bound to port 3008 - test framework fails silently trying to start its own server on occupied port (2026-02-23)
- **Situation:** Implementation was complete and properly structured, but could not verify by running test because dev server was blocking port. Per CLAUDE.md constraints, cannot stop dev server
- **Root cause:** Playwright tries to start its own test server on configured port, but OS prevents port reuse. Single port constraint + no dev server control = execution blocker
- **How to avoid:** Requires developer discipline: must stop dev server to run tests locally. But preserves test isolation and prevents accidental test runs against dev data

#### [Pattern] All verification tests share consistent structure: define apiCall helper with auth headers, organize tests in test.describe blocks, use environment-based configuration for API_BASE_URL and API_KEY (2026-02-23)
- **Problem solved:** Multiple API verification tests needed (agent-ops-smoke.spec.ts mirrors context-routes.spec.ts structure). Needed consistency for maintainability
- **Why this works:** Reduces boilerplate, makes tests scannable and familiar to developers, prevents copy-paste errors in auth/configuration, simplifies onboarding for new verification tests
- **Trade-offs:** Less flexibility in test structure, but dramatically easier to maintain and review. Pattern becomes discoverable through code inspection

### E2E tests maintain independent TypeScript validation path - only test file needs Prettier/TypeScript checks, not dependent on npm run build:packages passing (2026-02-23)
- **Context:** Pre-existing TypeScript error in libs/platform/src/secure-fs.ts blocked builds, but new test could still be validated and is structurally correct
- **Why:** Tests use separate TypeScript compilation scope (their own tsconfig in apps/ui). Separates concerns: package build validation ≠ test validity validation. Prevents unrelated infrastructure issues from blocking test development
- **Rejected:** Waiting for full build to pass before validating test (would couple test development to unrelated package issues, slowing velocity)
- **Trade-offs:** Tests need own validation checks (Prettier, tsc) but remain independent from package build. Two validation paths instead of one, but each path is focused
- **Breaking if changed:** If tests were tightly coupled to package build via shared tsconfig or build step, pre-existing errors would block all test work unrelated to those errors