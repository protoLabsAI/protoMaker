...
#### [Pattern] Platform-specific installation scripts (bash for macOS/Linux, PowerShell for Windows) replicate realistic user installation flow (DMG mount, NSIS silent install, AppImage +x permissions) rather than direct binary extraction (2026-02-25)
...
- **Why this works:** Smoke tests should catch installer-specific bugs (broken mount scripts, invalid NSIS configuration, missing permissions) before release. Users install via DMG/NSIS/APT, not raw extraction
...
- **Problem solved:** Smoke tests run in CI on GitHub runners without access to external services. App normally requires API calls to backend
- **Why this works:** Test reliability depends on not hitting external dependencies. Mock mode allows testing app startup, window rendering, and state persistence without API infrastructure. Trades complete integration testing for test isolation
- **Trade-offs:** Tests verify app can launch and setup flow completes, but don't verify API integration. Real API testing happens at system integration level

### Reuse existing Playwright test patterns from apps/ui/tests/electron/ rather than building custom test framework (2026-02-25)

- **Context:** Could have used different test runner (Cypress, custom Node script, etc)
- **Why:** Playwright already imported for Electron testing. Reusing patterns maintains consistency with existing test suite, avoids adding new dependency, and developers familiar with existing tests can modify smoke tests. Test patterns already battle-tested
- **Rejected:** Custom test harness (faster to write initially, but adds maintenance burden and diverges from team practices)
- **Trade-offs:** Bounded by Playwright's capabilities/idioms, but aligned with team's test infrastructure. Lower onboarding friction for future modifications
- **Breaking if changed:** Custom test framework would require different CI setup, different local development experience, different debugging tools
...
- **Why:** Builds are expensive to recreate and needed for emergency rollbacks. Test results are primarily for debugging the most recent failures. After 7 days, test logs are stale (new code landed), but builds might still be needed for patch releases
- **Rejected:** Uniform 14-day retention (wastes storage on old test logs, might not keep builds long enough)
- **Trade-offs:** Slightly higher complexity in CI configuration, but optimizes storage costs ($$ per GB-day) for artifact pattern (frequent test results, infrequent build retrieval)
...
#### [Pattern] Enforce performance minimums in CI/CD via Lighthouse CI with absolute score thresholds (95/100/100/100) (2026-02-25)

- **Problem solved:** Performance improvements made but needed guardrails to prevent regression on future changes
- **Why this works:** Performance debt accumulates invisibly - one change adds 20ms, another adds 30ms, and suddenly performance regresses. CI enforcement prevents this by failing builds when scores drop. Absolute thresholds (not relative) catch gradual degradation.
- **Trade-offs:** Strict thresholds may require developers to optimize before merging (slows feature velocity slightly), but prevents worse pain of performance incidents post-deployment. Cost-benefit heavily favors enforcement.
