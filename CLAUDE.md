...
When a feature blocks, check `statusChangeReason` and follow the recovery runbooks -> **[docs/internal/dev/recovery-runbooks.md](docs/internal/dev/recovery-runbooks.md)** (uncommitted-worktree commit failures, prettier drift, merge_conflict / stuck MERGE_HEAD, stale-context resume trap, wrong branch-prefix, stale-ESCALATE checkpoint).

**Self-improvement rule:** on a recurring agent-blocking failure, immediately (1) file a P1 bug, (2) add the pattern to `.automaker/memory/ops-lessons.md`, (3) add a recovery entry to the runbook doc. The flywheel only improves if failures are captured.
...
Docs follow the [Diataxis](https://diataxis.fr/) framework (tutorial / how-to / reference / explanation -- never mix types on one page). Full standard -- content principles, page template, the two surfaces (public VitePress `docs/` vs internal `docs/internal/`), and the TTFHW metric -> **[docs/internal/dev/documentation-design.md](docs/internal/dev/documentation-design.md)** and `docs/internal/dev/docs-standard.md`.
