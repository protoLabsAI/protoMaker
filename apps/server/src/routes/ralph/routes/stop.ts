...
const logger = createLogger('RalphRoutes:Stop');

export function createStopRalphLoopHandler({ ralphLoopService }: RalphDeps) {
  return async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      logger.info('Stopping Ralph loop', { featureId });

      const state = await ralphLoopService.stopLoop(featureId);
