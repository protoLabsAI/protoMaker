...
Decompose this milestone into implementable phases.`;
   ...
   */
  private async checkMilestoneCompletion(projectPath: string): Promise<void> {
    try {
      const projects = await this.projectService.listProjects(projectPath);
      ...
      for (const slug of projects) {
        ...
        for (let i = 0; i < project.milestones.length; i++) {
          ...
          // Check if all features for this milestone are done
          const allDone = await this.areMilestoneFeaturesComplete(projectPath, milestone);

          if (allDone) {
            logger.info(`Milestone "${milestone.title}" complete!`);
            ...
            const nextIndex = project.milestones.findIndex(
              (m) => m.status === 'stub' || m.status === 'pending'
            );

            if (nextIndex >= 0) {
              // Get the parent epic feature
              const features = await this.featureLoader.getAll(projectPath);
              const epic = features.find((f) => f.projectSlug === slug && f.isEpic);
              ...
              if (epic) {
                this.events.emit('milestone:planning-started', {
                  ...
                  projectTitle: project.title,
                  projectSlug: slug,
                  milestoneTitle: project.milestones[nextIndex].title,
                  milestoneNumber: nextIndex + 1,
                });

                await this.planMilestone(projectPath, slug, nextIndex, epic);
              }
            } else {
   ...
   */
  private async areMilestoneFeaturesComplete(
    projectPath: string,
    milestone: Milestone
  ): Promise<boolean> {
    ...
    for (const phase of milestone.phases) {
      ...
      const feature = features.find((f) => f.id === phase.featureId);
