...
export class AutoModeService {
  ...
  // FeatureScheduler handles pending feature loading, concurrency, and loop body
  private featureScheduler!: FeatureScheduler;
  ...
  constructor(events: EventEmitter, settingsService?: SettingsService) {
    ...
    // Initialize FeatureScheduler with the scheduling logic and PipelineRunner (this)
    const schedulerCallbacks: SchedulerCallbacks = {
      getRunningCountForWorktree: (projectPath: string, branchName: string | null): number =>
        this.concurrencyManager.getRunningCountForWorktree(projectPath, branchName),
      emitAutoModeEvent: this.emitAutoModeEvent.bind(this),
      trackFailureAndCheckPauseForProject: this.trackFailureAndCheckPauseForProject.bind(this),
      signalShouldPauseForProject: this.signalShouldPauseForProject.bind(this),
      canRunFeature: (feature) => {
        if (feature.featureType === 'content' && !this.leadEngineerService) {
          return false;
        }
        return true;
      },
      updateHumanBlockedCount: (projectPath: string, branchName: string | null, count: number) => {
        ...
        if (projectState) {
          projectState.humanBlockedCount = count;
        }
      },
    };
    this.featureScheduler = new FeatureScheduler(
      this as unknown as PipelineRunner,
      this.featureLoader,
      this.events,
      this.settingsService,
      this.runningFeatures,
  ...
  setFeatureHealthService(
    ...
    this.featureScheduler.setFeatureHealthService(service);
  }

  /**
   * PipelineRunner.run() implementation.
   * Called by FeatureScheduler when it selects a feature to execute.
   * Delegates to leadEngineerService.process() if available, otherwise executeFeature().
   */
  async run(projectPath: string, featureId: string): Promise<PipelineResult> {
    const feature = await this.featureLoader.get(projectPath, featureId);
    if (!feature) {
      throw new Error(`Feature ${featureId} not found in project ${projectPath}`);
    }
    const featureModelResult = await this.getModelForFeature(feature, projectPath);
    await (this.leadEngineerService
      ? this.leadEngineerService.process(projectPath, featureId, {
          model: featureModelResult.model,
        } as unknown as import('@protolabsai/types').ExecuteOptions)
      : this.executeFeature(projectPath, featureId, this.config?.useWorktrees ?? true, true));
  ...
  private async resolveMaxConcurrency(
    ...
    return this.featureScheduler.resolveMaxConcurrency(projectPath, branchName, provided);
  }
   ...
   * Delegates to FeatureScheduler.runLoop() which owns the loop body.
  ...
  private async runAutoLoopForProject(worktreeKey: string, loopState?: LoopState): Promise<void> {
    ...
    return this.featureScheduler.runLoop(projectState);
  }
...
Format your response as a structured markdown document.`;
   ...
   * Delegates to FeatureScheduler which owns the implementation.
   * @param projectPath - The project path
  ...
  private async loadPendingFeatures(
    ...
    return this.featureScheduler.loadPendingFeatures(projectPath, branchName);
  }
  /**
