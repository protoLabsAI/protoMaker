...
export class SensorRegistryService {
  ...
  /**
   * Poll system idle time from Electron's powerMonitor once.
   * Called immediately on startup and then on the scheduler interval
   * registered by scheduler.module.ts via registerInterval().
   *
   * If not running inside Electron the dynamic import fails silently and no
   * reading is produced (the sensor stays offline / stale).
   */
  async pollElectronIdle(): Promise<void> {
    try {
  ...
  /**
   * Start the built-in Electron idle poller.
   * Fires immediately once; subsequent polls are driven by the scheduler interval
   * registered in scheduler.module.ts.
   */
  private _startElectronIdlePoller(): void {
    void this.pollElectronIdle();
  }

  /** No-op — the scheduler interval is managed externally via registerInterval(). */
  stopBuiltinSensors(): void {
    // Interval is owned by schedulerService; nothing to clear here.
  }
