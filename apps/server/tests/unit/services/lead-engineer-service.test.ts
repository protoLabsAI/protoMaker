...
describe('LeadEngineerService', () => {
  ...
  describe('checkMergedPRs', () => {
    ...
    it('transitions review feature to done when PR is merged', async () => {
      const feature = createMockFeature({ id: 'f1', status: 'review', prNumber: 42 });
      ...
      const mergedAt = '2026-03-10T12:00:00Z';
      ...
      expect(featureLoader.update).toHaveBeenCalledWith('/test/project', 'f1', {
        status: 'done',
        prMergedAt: mergedAt,
      });
    ...
    it('does not update feature when PR state is OPEN', async () => {
      const feature = createMockFeature({ id: 'f1', status: 'review', prNumber: 42 });
      featureLoader = createMockFeatureLoader([feature]);
      service = new LeadEngineerService(
        events as any,
      ...
      expect(events.emit).not.toHaveBeenCalledWith('feature:pr-merged', expect.anything());
    });

    it('skips features without a prNumber', async () => {
      const feature = createMockFeature({ id: 'f1', status: 'review' });
      featureLoader = createMockFeatureLoader([feature]);
      service = new LeadEngineerService(
        events as any,
        featureLoader as any,
    ...
    it('skips features not in review status', async () => {
      const feature = createMockFeature({ id: 'f1', status: 'in_progress', prNumber: 42 });
      featureLoader = createMockFeatureLoader([feature]);
      service = new LeadEngineerService(
        events as any,
        featureLoader as any,
    ...
    it('handles gh CLI failure gracefully and continues to next feature', async () => {
      const f1 = createMockFeature({ id: 'f1', status: 'review', prNumber: 10 });
      const f2 = createMockFeature({ id: 'f2', status: 'review', prNumber: 11 });
      featureLoader = createMockFeatureLoader([f1, f2]);
      service = new LeadEngineerService(
        events as any,
        featureLoader as any,
        autoModeService as any,
      ...
      expect(featureLoader.update).toHaveBeenCalledTimes(1);
      expect(featureLoader.update).toHaveBeenCalledWith('/test/project', 'f2', {
        status: 'done',
        prMergedAt: mergedAt,
      });
    });

    it('does not run when session is stopped', async () => {
      await service.initialize();
      await service.start('/test/project', 'my-project');
      await service.stop('/test/project');
    ...
    it('uses current timestamp when mergedAt is null in gh response', async () => {
      const feature = createMockFeature({ id: 'f1', status: 'review', prNumber: 99 });
      featureLoader = createMockFeatureLoader([feature]);
      service = new LeadEngineerService(
        events as any,
        featureLoader as any,
      ...
      mockExec.mockResolvedValue({
        stdout: JSON.stringify({ state: 'MERGED', mergedAt: null }),
      });
      ...
      const updateCall = (featureLoader.update as ReturnType<typeof vi.fn>).mock.calls[0];
      const prMergedAt = updateCall[2].prMergedAt as string;
      expect(prMergedAt >= before).toBe(true);
      expect(prMergedAt <= after).toBe(true);
    });

    it('triggers PR merge polling interval every 2.5 minutes', async () => {
      const feature = createMockFeature({ id: 'f1', status: 'review', prNumber: 42 });
      featureLoader = createMockFeatureLoader([feature]);
      service = new LeadEngineerService(
        events as any,
