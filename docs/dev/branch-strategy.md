...
feature/* ──▶ dev ──────────────────────────────────── (playground)
              ...
              staging ──────────────────────────────── (integration / user QA)
                ...
                │
                │  PR from staging into main (approval + full CI required)
                │
               main ──────────────────────────────────── (stable release)
                         Tagged on every merge, never auto-deploys
```
...
| --------- | --------------------------------------------------- | --------------- | --------------------------- | ------------ |
| `main`    | Stable release — every commit is a tagged release   | Nobody directly | Yes — from `staging` only   | Never auto   |
| `staging` | Integration — user-testable, QA environment         | Nobody directly | Yes — from `dev` or `dev/*` | Auto on push |
...
3. CI runs + 1 approval required
4. **Enforcement**: The `promotion-check` CI job hard-fails any PR to `main` that doesn't originate from `staging`
5. On merge: Changesets auto-creates a version bump PR, then tags a GitHub Release
...
- Require 1 approval
- Require status checks: `build`, `test`, `checks`, `source-branch` (promotion-check)
- No direct push — even admins

**`staging`**

- Require PR before merging
- Require status checks: `build`, `test`, `checks`
- No direct push
