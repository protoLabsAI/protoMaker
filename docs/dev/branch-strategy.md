...
| `auto-release.yml`      | `staging→main` PR merged        | Version bump + `v*` tag + GitHub Release       |
| `build-electron.yml`    | `v*` tag push                   | Multi-platform Electron builds                 |
| `changeset-release.yml` | Push to `main` (commit-guarded) | Changeset version PR (non-auto release paths)  |
...
**How does the release happen?**
When a `staging→main` PR merges, `auto-release.yml` automatically runs `release:prepare` (analyzes conventional commits), bumps all `@protolabs-ai/*` packages, pushes a `chore: release vX.Y.Z` commit, pushes a `vX.Y.Z` tag (via `GH_PAT`), and creates a GitHub Release. The tag push triggers `build-electron.yml` for multi-platform Electron builds. No manual steps required. `changeset-release.yml` is guarded against the auto-release commit message to prevent double-firing.
