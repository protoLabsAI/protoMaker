...
`@protolabsai/types` is imported by both server and UI. Any `process.env` access at module import time crashes in the browser where `process` is undefined. Guard with `typeof process !== 'undefined'`.
