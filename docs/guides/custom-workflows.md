...
Three workflows are available without any YAML files:
...
phases:
  ...
  - state: PLAN
    enabled: true
    processor: my-custom-processor # Override the default processor for a phase
  - state: EXECUTE
    enabled: true
  - state: REVIEW
    enabled: true
...
Standard code workflow but with a custom processor overriding the PLAN phase:

```yaml
name: custom-plan-code
description: Code features that need a custom planning phase
phases:
  - state: INTAKE
    enabled: true
  - state: PLAN
