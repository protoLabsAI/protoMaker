...
The `promotion-check.yml` workflow normally requires all PRs to `main` to originate from `staging`. Four named exceptions exist:

| Pattern                                  | Purpose                                            |
| ---------------------------------------- | -------------------------------------------------- |
| `chore/branch-strategy-implementation`   | Bootstrap PR — strategy not in staging yet         |
| `chore/promote-staging-main-*`           | DAG recovery after accidental squash promotion     |
| `fix/fixci-*`                            | CI fix bypass — repairs a broken CI gate to `main` |
| `feature/fixci-*`                        | Transitional alias for `fix/fixci-*`               |

When creating a CI fix branch, prefer `fix/fixci-{pr-number}-{short-description}`. The `feature/fixci-*` pattern is also accepted for backwards compatibility.
