...
Six fleet agents organized into three teams, **all hosted in protoWorkstacean** (the portfolio brain). protoMaker is the per-project execution target these agents drive — it is not itself a fleet agent. All A2A agents expose `/.well-known/agent.json` and accept JSON-RPC 2.0 `message/send` calls.

| Agent          | Team      | Role                                                                      | A2A Endpoint             | Discord Bot                 | Key Skills                                                                                        |
| -------------- | --------- | ------------------------------------------------------------------------- | ------------------------ | --------------------------- | ------------------------------------------------------------------------------------------------- |
| **Ava**        | Dev       | Portfolio orchestrator — planning, cross-app authority (protoWorkstacean) | protoWorkstacean (A2A)   | `protoava[bot]`             | `plan`, `plan_resume`, `sitrep`, `manage_feature`, `auto_mode`, `board_health`, `onboard_project` |
...
| **Orchestration + Planning** | `protoWorkstacean` | The portfolio brain: message routing, skill matching, interface plugins, `workspace/agents.yaml`, `workspace/projects.yaml`, `GET /api/*` endpoints, `POST /publish`, HITL gate routing, correlationId minting. **Plus** the planning pipeline (`plan`/`plan_resume` skills), SPARC PRD generation, antagonistic review orchestration, SQLite plan checkpoints, reflection/ceremonies, and the **Ava** portfolio orchestrator. |
| **Execution**                | `protoMaker`       | Per-project execution: board management, Lead Engineer state machine, auto-mode, agent execution, the PR pipeline (single guarded chokepoint `runPostCompletionWorkflow`). Driven on a per-project basis by the **Roxy** CLI operator.                                                                                                                                                                                         |

### What Each Repo Does NOT Own

- **homelab-iac** does not contain agent logic, skill definitions, or routing rules
- **protoWorkstacean** does not manage per-project board state or run feature-execution agents — it delegates that to protoMaker
- **protoMaker** is a pure executor: it does not run the planning pipeline, antagonistic review, the Ava orchestrator, or any autonomous cross-project loop. It does not define the agent/project registry (reads from Workstacean API) and does not handle message routing (publishes/subscribes via bus).
