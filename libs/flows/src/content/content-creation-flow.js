...
import { createSectionWriterGraph } from './subgraphs/section-writer.js';
...
export const ContentCreationState = Annotation.Root({
  ...
  researchQueries: Annotation,
  researchResults: Annotation({
    reducer: (left, right) => [...left, ...right],
    default: () => [],
  }),
  researchApproved: Annotation,
  researchFeedback: Annotation,
  // Phase 2: Outline
  outline: Annotation,
  outlineApproved: Annotation,
  outlineFeedback: Annotation,
  ...
  // Phase 4: Assembly
  assembledContent: Annotation,
  // Phase 5: Review (parallel)
  reviewFeedback: Annotation({
    reducer: (left, right) => [...left, ...right],
    default: () => [],
  }),
  reviewApproved: Annotation,
  finalReviewFeedback: Annotation,
 ...
 */
async function generateQueriesNode(state) {
  const { config } = state;
 ...
 */
async function fanOutResearchNode(state) {
 ...
 */
async function researchDelegateNode(state) {
  const { query } = state;
 ...
 */
async function researchHitlNode(state) {
 ...
 */
function routeAfterResearchHitl(state) {
 ...
 */
async function generateOutlineNode(state) {
 ...
 */
async function outlineHitlNode(state) {
  const { outline } = state;
 ...
 */
function routeAfterOutlineHitl(state) {
 ...
 */
async function fanOutGenerationNode(state) {
  const { outline } = state;
  if (!outline) {
 ...
 */
async function generationDelegateNode(state) {
 ...
 */
async function assembleNode(state) {
 ...
 */
async function fanOutReviewNode(state) {
 ...
 */
async function reviewDelegateNode(state) {
  const { section } = state;
 ...
 */
async function finalReviewHitlNode(state) {
 ...
 */
function routeAfterFinalReviewHitl(state) {
 ...
 */
async function fanOutOutputNode(state) {
  const { config } = state;
  const sends = [];
 ...
 */
async function outputDelegateNode(state) {
 ...
 */
export function createContentCreationFlow(config) {
...
//# sourceMappingURL=content-creation-flow.js.map
