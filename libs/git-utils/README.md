...
```bash
npm install @protolabsai/git-utils
```
...
```typescript
import { parseGitStatus } from '@protolabsai/git-utils';
import type { FileStatus } from '@protolabsai/git-utils';
...
```typescript
import { listAllFilesInDirectory, generateDiffsForNonGitDirectory } from '@protolabsai/git-utils';
...
```typescript
import { isGitRepo, getGitRepositoryDiffs, parseGitStatus } from '@protolabsai/git-utils';
...
try {
  ...
  // Errors are logged via @protolabsai/utils logger
...
- `@protolabsai/types` - FileStatus type definition
- `@protolabsai/utils` - Logger utilities

## Used By

- `@protolabsai/server` - Git routes, worktree operations, feature context
