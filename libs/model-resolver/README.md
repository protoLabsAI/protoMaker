...
```bash
npm install @protolabsai/model-resolver
```
...
```typescript
import { getEffectiveModel } from '@protolabsai/model-resolver';
...
```typescript
import { DEFAULT_MODELS } from '@protolabsai/model-resolver';
import { CLAUDE_MODEL_MAP } from '@protolabsai/types';
...
- `@protolabsai/types` - Model type definitions and constants

## Used By

- `@protolabsai/server` - Feature execution, agent chat, enhancement
