...
// ============================================================================
// Ralph Loop Paths (persistent retry loops with external verification)
// ============================================================================

/**
 * Get the Ralph directory for a feature
 *
 * Contains Ralph loop state and progress logs.
 *
 * @param projectPath - Absolute path to project directory
 * @param featureId - Feature identifier
 * @returns Absolute path to {projectPath}/.automaker/features/{featureId}/ralph
 */
export function getRalphDir(projectPath: string, featureId: string): string {
  return path.join(getFeatureDir(projectPath, featureId), 'ralph');
}

/**
 * Get the Ralph state file path
 *
 * Stores the current Ralph loop state JSON.
 *
 * @param projectPath - Absolute path to project directory
 * @param featureId - Feature identifier
 * @returns Absolute path to {projectPath}/.automaker/features/{featureId}/ralph/ralph-state.json
 */
export function getRalphStatePath(projectPath: string, featureId: string): string {
  return path.join(getRalphDir(projectPath, featureId), 'ralph-state.json');
}

/**
 * Get the Ralph progress log file path
 *
 * Stores the human-readable progress log.
 *
 * @param projectPath - Absolute path to project directory
 * @param featureId - Feature identifier
 * @returns Absolute path to {projectPath}/.automaker/features/{featureId}/ralph/ralph-progress.md
 */
export function getRalphProgressPath(projectPath: string, featureId: string): string {
  return path.join(getRalphDir(projectPath, featureId), 'ralph-progress.md');
}

/**
 * Create the Ralph directory for a feature if it doesn't exist
 *
 * @param projectPath - Absolute path to project directory
 * @param featureId - Feature identifier
 * @returns Promise resolving to the created ralph directory path
 */
export async function ensureRalphDir(projectPath: string, featureId: string): Promise<string> {
