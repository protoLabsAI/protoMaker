...
import type { ProtoConfig } from '@protolabsai/types';
...
export async function loadProtoConfig(projectPath: string): Promise<ProtoConfig | null> {
  ...
  const base: ProtoConfig = (parseYaml(raw) as ProtoConfig | null) ?? ({ name: '' } as ProtoConfig);
 ...
 * Used by setup_lab to generate the initial configuration file.
 *
 * @param projectPath - Absolute path to the project root directory
