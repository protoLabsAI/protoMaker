...
```typescript
import type { Feature, FeatureStatus, PlanningMode, PlanSpec } from '@protolabsai/types';
```
...
```typescript
import type { ErrorType, ErrorInfo } from '@protolabsai/types';
```
...
```typescript
import type { ImageData, ImageContentBlock } from '@protolabsai/types';
```
...
```typescript
import { CLAUDE_MODEL_MAP, DEFAULT_MODELS, type ModelAlias } from '@protolabsai/types';
```
...
**IMPORTANT**: This package must NEVER depend on other `@protolabsai/*` packages to prevent circular dependencies. All other packages depend on this one, making it the foundation of the dependency tree.
...
1. **Never add dependencies** to other `@protolabsai/*` packages in `package.json`
2. **Keep result types here** - For example, `DependencyResolutionResult` should stay in `@protolabsai/dependency-resolver`, not be moved here
...
```
@protolabsai/types (foundation - no dependencies)
    ↓
@protolabsai/utils, @protolabsai/platform, etc.
    ↓
@protolabsai/server, @protolabsai/ui
```
