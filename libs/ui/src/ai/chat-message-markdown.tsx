/**
 * ChatMessageMarkdown — Streaming markdown renderer with word-by-word animation.
 *
 * Uses Streamdown for flicker-free streaming with fade-in animation on new
 * content. Handles unterminated markdown blocks mid-stream without artifacts.
 *
 * Citation support:
 *  - [[feature:id]] and [[doc:path]] markers in content are replaced with
 *    inline <span class="citation"> elements.
...
export interface ChatMessageMarkdownProps {
  ...
  citations?: Citation[];
  /** When true, shows streaming animation and caret. */
  isStreaming?: boolean;
}
...
export function ChatMessageMarkdown({
  ...
  // Pre-process citation markers
  const processedContent = useMemo(() => preprocessCitations(content), [content]);

  // Custom components — Streamdown uses the same components API as react-markdown
  const components = useMemo<Components>(() => {
    const citationsRef = citations;
    return {
      a: ({ ...props }) => (
        <a
          {...props}
      ...
      span: ({ className: spanClass, ...props }) => {
        if (spanClass === 'citation') {
          ...
          const resolved = citationsRef?.find(
  ...
  return (
    <div data-slot="chat-message-markdown" className={cn(...PROSE_CLASSES, className)}>
      <Streamdown
        components={components}
        parseIncompleteMarkdown={isStreaming}
        animated={
          isStreaming ? { animation: 'fadeIn', duration: 300, stagger: 20, sep: 'word' } : false
        }
