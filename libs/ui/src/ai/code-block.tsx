...
// ── Prism lazy-load helpers ──────────────────────────────────────────────────
// We use a tiny wrapper so server-side / test environments that lack a DOM
// don't explode when Prism tries to auto-detect the environment.
...
async function getPrism(): Promise<typeof import('prismjs') | null> {
  ...
  try {
    ...
    // Disable automatic highlighting on load (we call it manually)
    Prism.manual = true;

    // Load the most common grammars. These are small and tree-shakeable.
 ...
 * e.g. "js" → "javascript", "ts" → "typescript", "sh" → "bash".
 */
