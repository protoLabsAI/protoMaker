...
import type { RepoResearchResult, GapAnalysisReport, GapItem, ComplianceItem } from '../types.js';
...
export function analyzeGaps(
  ...
  let gapWeight = 0;
  for (const g of gaps) {
    const w = SEVERITY_WEIGHTS[g.severity as keyof typeof SEVERITY_WEIGHTS];
    totalWeight += w;
    gapWeight += w;
  }
