...
class LinearMCPClient {
  ...
  async getIssueDetails(issue) {
    const [statePromise, assigneePromise, teamPromise] = [issue.state, issue.assignee, issue.team];
  ...
  addMetricsToResponse(response) {
    ...
    // Fix: preserve arrays instead of spreading them into objects
    if (Array.isArray(response)) {
      return Object.assign(response, { metadata: { apiMetrics } });
    }
    return {
      ...response,
      metadata: {
  ...
  async getViewer() {
    ...
    const [teams, organization] = await Promise.all([viewer.teams(), this.client.organization]);
    return this.addMetricsToResponse({
      id: viewer.id,
      name: viewer.name,
      email: viewer.email,
  ...
  async getOrganization() {
    ...
    const [teams, users] = await Promise.all([organization.teams(), organization.users()]);
    return this.addMetricsToResponse({
      id: organization.id,
      name: organization.name,
      urlKey: organization.urlKey,
  ...
  buildSearchFilter(args) {
    const filter = {};
    if (args.query) {
      filter.or = [{ title: { contains: args.query } }, { description: { contains: args.query } }];
    }
    if (args.teamId) {
...
const resourceTemplates = [
  ...
  },
  {
    ...
    examples: ['linear-user:///USER-123/assigned', 'linear-user:///me/assigned'],
  },
];
const serverPrompt = {
...
const GetUserIssuesArgsSchema = z.object({
  userId: z
    .string()
    .optional()
    .describe("Optional user ID. If not provided, returns authenticated user's issues"),
