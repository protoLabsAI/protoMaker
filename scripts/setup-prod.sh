#!/bin/bash
# Automaker Production Setup Script (single-machine, Docker)
# One-command setup for a production environment with high-concurrency agent execution.
#
# Usage:
#   ./scripts/setup-prod.sh              # Full setup (build + start)
#   ./scripts/setup-prod.sh --build      # Rebuild images only
#   ./scripts/setup-prod.sh --start      # Start without rebuilding
#   ./scripts/setup-prod.sh --drain      # Gracefully stop agents before deploy
#   ./scripts/setup-prod.sh --stop       # Stop services
#   ./scripts/setup-prod.sh --status     # Show status
#   ./scripts/setup-prod.sh --teardown   # Stop and remove volumes
#
# Uses .env for API keys. Creates one if missing.
# For a non-Docker prod-like local instance (host CLIs reachable), use `npm start`.
...
AUTOMAKER_API_KEY=${AUTOMAKER_API_KEY:-automaker-prod-$(openssl rand -hex 8)}
GH_TOKEN=${GH_TOKEN:-}
...
start_services() {
  ...
  # Reset board state — features are gitignored runtime artifacts.
  # The server modifies .automaker/features/ via bind mount. Without
